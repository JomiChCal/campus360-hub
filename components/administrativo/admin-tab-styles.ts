export const ADMIN_TABS_LIST_CLASS =
  'mb-5 flex !h-auto w-full flex-wrap gap-1 overflow-x-auto rounded-2xl border-[var(--svc-border-hairline)] border-[color:var(--svc-color-border-subtle)] bg-[color:var(--svc-color-surface-subtle)] p-1.5 [scrollbar-width:none] group-data-[orientation=horizontal]/tabs:!h-auto sm:grid sm:grid-cols-3 [&::-webkit-scrollbar]:hidden';
...
export const ADMIN_TABS_TRIGGER_CLASS =
  'inline-flex !h-auto min-w-max flex-none items-center justify-center rounded-xl border-[var(--svc-border-hairline)] border-transparent bg-transparent px-4 py-2.5 text-[var(--svc-text-xs)] font-medium tracking-[0.06em] text-[color:var(--svc-color-text-muted)] uppercase transition-colors after:hidden hover:border-[color:var(--svc-color-border-subtle)] hover:bg-[color:var(--svc-color-surface-elevated)] hover:text-[color:var(--svc-color-text-primary)] data-[active]:border-[color:var(--svc-color-border-strong)] data-[active]:bg-[color:var(--svc-color-surface-elevated)] data-[active]:text-[color:var(--svc-color-text-primary)] data-[active]:shadow-sm group-data-[orientation=horizontal]/tabs:!h-auto sm:min-w-0 sm:flex-1';
